...
import { FolderOpen, Folder, ChevronRight, Copy as CopyIcon } from "lucide-react"
...
export function CopyDialog({ open, onOpenChange, file, source, onSuccess }: CopyDialogProps) {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setFolders([
          { id: "root", name: "Azure Storage", path: "", children: folderNodes },
        ])
      } catch (err) {
